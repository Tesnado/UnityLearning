...
public class UFOFactory { //开始时继承了MonoBehaviour后来发现不继承也可以
    ...
    private Dictionary<int, UFO> used = new Dictionary<int, UFO>();//正在使用的飞碟 
    private List<UFO> ufoPool = new List<UFO>();//飞碟对象池
    ...
    public UFO GetUFO(int round)  
    {
        ...
        if (ufodata == null)
        {
            ufodata = newUFO.AddComponent<UFO>();
        }
        ...
        ufodata.name = "ufo" + ufodata.GetInstanceID().ToString();
        return ufodata;  
    }
}
